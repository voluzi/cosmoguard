...
**The L2 (olric) cap is approximate by construction** — unlike the exact L1 byte bound. olric evicts by *sampled* LRU and cannot hold a partition below one entry, so each cache DMap has a floor of roughly `ownedPartitions × maxEntrySize`, and the aggregate L2 floor is `N_dmaps × ownedPartitions × 256 KiB`. Embedded/single-pod mode uses a reduced partition count so that floor fits the L2 budget of the smallest supported pod (~128 MiB with all proxies enabled); **pods smaller than that — or with unusually many cache DMaps — can exceed the resolved L2 budget by this floor.** Size the pod so its L2 budget clears the floor, or raise the memory limit. Entries larger than the 256 KiB fragment `tableSize` are served uncached rather than stored. Clustered deployments divide the per-node cap by `replicaCount`, since each node also holds replica copies that olric's primary-write cap doesn't govern. The L1 byte cap and `GOMEMLIMIT` are unaffected by any of this.
