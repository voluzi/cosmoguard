...
func (c *CORSConfig) ApplyToResponse(headers http.Header, requestOrigin string) {
	...
	// cache admission uses the raw upstream Vary captured before this method,
	// while this synthetic field is retained for downstream caches. addVary is
	// idempotent, so an upstream that already sent Vary: Origin isn't duplicated.
